...
#         $i,$fat[$i],$rami[$i],$rom[$i],$dam[$i],$resum[$i],$res[$i],$red[$i],$reda[$i],$resti[$i],$tisi[$i],$badec[$i],$basha[$i];
         $i,$fat[$i],$ram[$i],$phs[$i],$dam[$i],$red[$i],$res[$i],$tide[$i],$tish[$i],$badec[$i],$basha[$i],$fani[$i],$deni[$i],$ampd[$i],$ampa[$i],$rom[$i],$taf[$i],$tar,$sar,$dar;
  $suma = $suma + $res[$i];
  $sumt = $sumt + $tish[$i];
  if ($res[$i] > $maxram) {
    $maxram = $res[$i];
