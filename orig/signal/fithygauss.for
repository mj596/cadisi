      ...
      DIMENSION T(500),S(500),DT(500),DS(500),RHO(500)

      CHARACTER*80 c_line,infile

      open(12,file='fithygauss.res')
      open(13,file='fithygauss.dat')
...
C header for output of results
        ...
        WRITE(12,'(3(A,I3),A,4I2)')' N = ',N,', NR = ',NR,
     +  ', NRED = ',NRED,', LIST = ',LIST
        WRITE(12,'(A,4(G12.5,2x))')' first approx.: X = ',X

        WRITE(*,*)' performing fit with LSQGEN'
        WRITE(*,'(3(A,I3),A,4I2)')' N = ',N,', NR = ',NR,
     +  ', NRED = ',NRED,', LIST = ',LIST
...
C output of results
        WRITE(12,'(A,G12.4,A,I3,/,A,(4(G12.5,2x)))')
     ...
     +  ' result of fit: R = ',R,', NSTEP =',NSTEP,' X =',X
      ...
      DO 30 i=1,1001
        xx = i
        hysig = 2.0D0*X(3)*X(3)+(xx-X(2))*X(4)
        yy = X(1)*DEXP(-(xx-X(2))*(xx-X(2))/hysig)
        IF (hysig .LT. 0.0) yy = 0.0
        WRITE(13,*) xx,yy
30    CONTINUE       
...
C line to measurements with errors in abscissa and ordinate
      hs = 2.0D0*X(3)*X(3) + (ETA(2*K-1)-X(2))*X(4)
      LSQGFN=ETA(2*K)-X(1)*DEXP(-(ETA(2*K-1)-X(2))*(ETA(2*K-1)-X(2))/hs
     *)
      IF (hs .LT. 0.0) LSQGFN = ETA(2*K)
      END
