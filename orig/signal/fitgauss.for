      ...
      PARAMETER(MAXN=2000,MAXNR=3,MAXM=1000)
      ...
      DIMENSION A2(MAXN+MAXNR,MAXN+MAXNR),LIST(3)
      DIMENSION T(100),S(100),DT(100),DS(100),RHO(100)

      CHARACTER*80 c_line

      open(11,file='spehisto.dat')
      open(12,file='fitgauss.out')
      open(13,file='fitgauss.dat')

      NP = 0
      NRD = 0

C      read(1,*) ersk1, ersk2, roh1, roh2
      ersk1 = 2.0
      ersk2 = 0.01
      roh1 = 0.0
      roh2 = 0.0

C      CALL GETARG(1,c_line)
C      WRITE(*,*) c_line
C      READ(c_line,*) X(1)
C      CALL GETARG(2,c_line)
C      READ(c_line,*) X(2)
      ...
      read(11,*,end=3)  TTEMP, STEMP, DTTEMP

C      if(ISK.eq.0) go to 4

      NRD = NRD + 1

      IF (NRD .LT. NP1 .OR. NRD .GT. NP2) GO TO 4
...
C identify program to user
      WRITE(12,*)' Program E7LSQ demonstrates use of LSQGEN'
      WRITE(12,*)' '
C write table of data
      WRITE(12,*)'    T         S         DT        DS        RHO'
      WRITE(*,*)'    T         S         DT        DS        RHO'
      DO 5 I=1,NP
        WRITE(12,'(5F10.3)')T(I),S(I),DT(I),DS(I),RHO(I)
...
C header for output of results
        WRITE(12,*)' performing fit with LSQGEN'
        WRITE(12,'(3(A,I2),A,3I2)')' N = ',N,', NR = ',NR,
     +  ', NRED = ',NRED,', LIST = ',LIST
        WRITE(12,'(A,3(G12.5,2x))')' first approx.: X = ',X

        WRITE(*,*)' performing fit with LSQGEN'
        WRITE(*,'(3(A,I2),A,3I2)')' N = ',N,', NR = ',NR,
     +  ', NRED = ',NRED,', LIST = ',LIST
        WRITE(*,'(A,3(G12.5,2x))')' first approx.: X = ',X

        NSTEP=100
        CALL LSQGEN(Y,CY,GY,F,E,M,N,NR,NRED,LIST,X,CX,R,A2,NSTEP)
C output of results
        WRITE(12,'(A,G12.4,A,I3,/,A,(3(G12.5,2x)))')
     +  ' result of fit: R = ',R,', NSTEP =',NSTEP,' X =',X

        WRITE(*,'(A,G12.4,A,I3,/,A,(3(G12.5,2x)))')
     +  ' result of fit: R = ',R,', NSTEP =',NSTEP,' X =',X

        IF(NRED.GT.0) THEN
          WRITE(2,*)' covariance matrix CX = '
          ...
          write(12,121) sig1, sig2, sig3
          write(12,122) coref12, coref13, coref23
      ...
      DO 30 i=1,1001
        xx = i
        yy = X(1)*DEXP(-(xx-X(2))*(xx-X(2))/(2.D0*X(3)*X(3)))
        WRITE(13,*) xx,yy
30    CONTINUE       
...
C line to measurements with errors in abscissa and ordinate
      LSQGFN=ETA(2*K)-X(1)*DEXP(-(ETA(2*K-1)-X(2))*(ETA(2*K-1)-X(2))/(2.
     *D0*X(3)*X(3)))
      END
